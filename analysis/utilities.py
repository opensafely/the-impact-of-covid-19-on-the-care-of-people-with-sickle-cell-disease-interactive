...
def plot_measures(
    ...
    if category:
        for unique_category in sorted(df[category].unique()):
            if (
                unique_category != "missing" and unique_category != "Unknown"
            ):  # Don't plot where catagory missing or unknown
                df_subset = df[df[category] == unique_category].sort_values(
                    "date"
                ...
                if deciles:
                    if unique_category == 50:
                        ...
                        linestyle = "b--"
                    plt.plot(
                        df_subset["date"], df_subset[column_to_plot], linestyle
                    )  # Plot decile
                else:
                    plt.plot(
                        df_subset["date"], df_subset[column_to_plot]
                    )  # Plot category
    else:
        if as_bar:
...
def binary_care_home_status(
    df, numerator_column: str, denominator_column: str, valuecolname: str = "value"
):
    ...
    grouped_df = df.groupby(["care_home_type", "date"], as_index=False)[
        [numerator_column, denominator_column]
    ].sum()
    grouped_df[valuecolname] = (
        grouped_df[numerator_column] / grouped_df[denominator_column]
    )
    return grouped_df
